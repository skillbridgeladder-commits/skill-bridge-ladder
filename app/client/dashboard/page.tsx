...
export default function ClientDashboard() {
  ...
  // NEW: Handle completing a job with Payment Logic
  const handleComplete = async (contract: any) => {
    if(!confirm("Are you sure you want to release payment ($" + contract.budget + ") and finish the job?")) return;
    ...
    try {
      ...
      // 2. Create Payment Record (The Money Move)
      const { error: paymentError } = await supabase
        .from('payments')
        .insert({
          contract_id: contract.id,
          payer_id: user.id,            // From Client
          payee_id: contract.freelancer_id, // To Freelancer
      ...
      alert("✅ Payment Released Successfully! The job is now closed.")
      
      // Update UI: Remove the completed contract from the list
